...
cleanup_firstmate_home_children() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    rm -f "$sub_state/$child_id.status" "$sub_state/$child_id.turn-ended" "$sub_state/$child_id.check.sh" "$sub_state/$child_id.pr-url" "$sub_state/$child_id.meta" "$sub_state/$child_id.pi-ext.ts"
  done
}
...
rm -f "$STATE/$ID.status" "$STATE/$ID.turn-ended" "$STATE/$ID.check.sh" "$STATE/$ID.pr-url" "$STATE/$ID.meta" "$STATE/$ID.pi-ext.ts"
